...
// pill shape defaults (from shape-context shapeMap.pill)
const shape = {
  bg: "rounded-[20px]",
  input: "rounded-[20px]",
} as const;
...
const Slider = forwardRef<HTMLDivElement, SliderProps>(
  ({
    ...
    const computeHoverPreview = useCallback((cursorX: number, trackWidth: number) => {
      const localX = cursorX - THUMB_SIZE / 2;
      const clamped = Math.max(0, Math.min(trackWidth - THUMB_SIZE, localX));
      const snappedVal = pixelToValue(clamped, min, max, step, trackWidth);
      const snappedPx = valueToPixel(snappedVal, min, max, trackWidth);
      const snappedX = snappedPx + THUMB_SIZE / 2;
